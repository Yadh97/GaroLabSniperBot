import os

# -----------------------
# Sniper Filtering Config
# -----------------------

# Filter thresholds
MIN_LIQUIDITY_USD = 20000         # Minimum liquidity in USD
MAX_FDV_USD = 500000              # Maximum fully diluted valuation
TOP_HOLDER_MAX_PERCENT = 5.0      # Top holder limit (%)

# -----------------------
# API and Network Config
# -----------------------

# Solana RPC endpoint
RPC_URL = os.getenv("RPC_URL", "https://api.mainnet-beta.solana.com")

# RugCheck API (public access endpoint)
RUGCHECK_BASE_URL = "https://api.rugcheck.xyz/v1/tokens"

# Moralis (for Pump.fun token list)
MORALIS_API_KEY = os.getenv("MORALIS_API_KEY", "")
PUMPFUN_NEW_TOKENS_URL = "https://solana-gateway.moralis.io/token/mainnet/exchange/pumpfun/new"

# DexScreener new token listing
DEXSCREENER_NEW_PAIRS_URL = "https://dexscreener.com/new-pairs"

# SOL mint address (Jupiter format)
SOL_MINT_ADDRESS = "So11111111111111111111111111111111111111112"

# -----------------------
# Telegram Bot Config
# -----------------------

TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")

# -----------------------
# Auto-Buy Settings
# -----------------------

AUTO_BUY = os.getenv("AUTO_BUY", "0") == "1"
TRADE_SIZE_SOL = float(os.getenv("TRADE_SIZE_SOL", "1.0"))        # in SOL
SLIPPAGE_BPS = int(os.getenv("SLIPPAGE_BPS", "50"))               # slippage in bps
TAKE_PROFIT_MULTIPLE = float(os.getenv("TAKE_PROFIT_MULTIPLE", "2.0"))  # e.g. 2.0 = 2x
POLL_INTERVAL = float(os.getenv("POLL_INTERVAL", "5"))            # how often to poll in seconds

# -----------------------
# Wallet Key
# -----------------------

PRIVATE_KEY = os.getenv("PRIVATE_KEY", "")
